...
namespace QN.Ots.Photo.Core.Service
{
    ...
    public class ApiUserService : IApiUserSI
    {

        private ApiUserBusinessComponent bc = QNInjection.Build<ApiUserBusinessComponent>();

        private IHttpContextAccessor http = QNInjection.Build<IHttpContextAccessor>();

        public BizData<(List<ApiUserBiz>, int)> GetApiUserPaging(PageRequest<ApiUserBiz> data)
        {
            var boData = data.Transfer<PageRequest<ApiUserBO>>();
            var result = bc.GetApiUserPaging(boData);

            var bizResponse = new BizData<(List<ApiUserBiz>, int)>()
            {
                Code = 200,
                Success = true,
                Msg = "succeed",
                Data = (result.Item1.Transfer<List<ApiUserBiz>>(), result.Item2)
            };

            return bizResponse;
        }

        public BizData<List<ApiUserBiz>> GetApiUser(QueryRequest<ApiUserBiz> data)
        {
            var boData = data.Transfer<QueryRequest<ApiUserBO>>();
            var result = bc.GetApiUser(boData);

            var bizResponse = new BizData<List<ApiUserBiz>>()
            {
                Code = 200,
                Success = true,
                Msg = "succeed",
                Data = result.Transfer<List<ApiUserBiz>>()
            };

            return bizResponse;
        }

        public BizData<ApiUserBiz> GetApiUserDetail(ApiUserBiz data)
        {
            var list = GetApiUser(new QueryRequest<ApiUserBiz>() { Data = data });

            var bizResponse = new BizData<ApiUserBiz>()
            {
                Code = 200,
                Success = true,
                Msg = "succeed",
        ...
        public BizData<int> AddApiUser(List<ApiUserBiz> data)
        {
            var boData = data.Transfer<List<TEdfApiUser>>();
            var result = bc.AddApiUser(boData);
            return new BizData<int>() { Code = 200, Success = true, Msg = "succeed", Data = result };
        }

        public BizData<int> UpdateApiUser(List<ApiUserBiz> data)
        {
            var boData = data.Transfer<List<TEdfApiUser>>();
            var result = bc.UpdateApiUser(boData);
            return new BizData<int>() { Code = 200, Success = true, Msg = "succeed", Data = result };
        }

        public BizData<int> DeleteApiUser(QNIDParamBiz data)
        {
            var boData = data.Transfer<QNIDParam>();
            var result = bc.DeleteApiUser(boData);
            return new BizData<int>() { Code = 200, Success = true, Msg = "succeed", Data = result };
        }

        public BizData<int> DataStatusApiUser(QNIDParamBiz data)
        {
            var boData = data.Ids.Select(x => new ApiUserBiz() { Id = x.Id, DataStatus = x.IsPhysical }).ToList();
            var result = UpdateApiUser(boData);
            return new BizData<int>() { Code = 200, Success = true, Msg = "succeed", Data = result.Data };
        }

        public BizData<QNExcelExportData> ExportApiUser(PageRequest<ApiUserBiz> data)
        {
            //1.获取数据
            List<ApiUserBiz> exportData = GetApiUserPaging(data).Data.Item1;

            //2.拼装参数
            var env = QNInjection.Build<IHostingEnvironment>();

            var fileName = $"ApiUserBiz_{DateTime.Now.ToString("yyyymmddHHmmss") + DateTime.Now.Millisecond + QNUtils.RandNum(4)}.xlsx";

            var filePath = env.ContentRootPath + "\\wwwroot\\excel\\download\\";
            var fullPath = $"{filePath}{fileName}";

            QNExcelExportModel<ApiUserBiz> exportParam = new QNExcelExportModel<ApiUserBiz>()
            {
        ...
        public BizData<QNUserInfoBiz> LoginApiUser(QueryRequest<LoginBiz> data)
        {
            ...
            BizData<List<ApiUserBiz>> mngs = new BizData<List<ApiUserBiz>>();

            if (data.Data.Type == "account")
            {
                var par = new QueryRequest<ApiUserBiz>()
                {
                    Data = new ApiUserBiz() { UserName = data.Data.UserName }
                };
            ...
            if (data.Data.Type == "mobile")
            {
                var par = new QueryRequest<ApiUserBiz>()
                {
                    Data = new ApiUserBiz() { Phone = data.Data.Mobile }
                };
        ...
        public BizData<QNUserInfoBiz> LoginAndCheck(LoginBiz par, ApiUserBiz cusInfo, string logPhoneType)
        {
        ...
        public BizData<QNUserInfoBiz> GetCurrentApiUser()
        {
            QNUserInfoBiz sysUser = PhotoHelper.GetCurrentUser();
            BizData<ApiUserBiz> user = GetApiUserDetail(new ApiUserBiz() { Id = sysUser?.Id ?? "/" });
