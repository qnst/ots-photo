import { listBooking, addBooking, deleteBooking, updateBooking, exportBooking } from './service';
...
const Index: React.FC = () => {
  ...
  const Add = async (fields: any) => {
    const hide = message.loading(intl.formatMessage({ id: 'mnt.booking.msg.add.loading' }));
    return await addBooking({ ...fields }).then(rsp => {
      hide();
      messageApi.open({
  ...
  const Update = async (fields: any) => {
    const hide = message.loading(intl.formatMessage({ id: 'mnt.booking.msg.update.loading' }));
    return await updateBooking({ ...fields }).then(rsp => {
      hide();
      messageApi.open({
        type: rsp.success ? 'success' : 'error', content: rsp.msg, duration: 3, className: '-', style: { marginTop: '6vh' },
      });
  ...
  const Delete = async (selectedRows: any) => {
    const hide = message.loading(intl.formatMessage({ id: 'mnt.booking.msg.delete.loading' }));
    if (!selectedRows) return true;

    return await deleteBooking({ ids: selectedRows.map((row: any) => { return { id: row.id } }) }).then(rsp => {
      hide();
      messageApi.open({
        type: rsp.success ? 'success' : 'error', content: rsp.msg, duration: 3, className: '-', style: { marginTop: '6vh' },
      });
  ...
  const Export = async (record: any, formRef: any, advanceSearch: any) => {
    const hide = message.loading(intl.formatMessage({ id: 'mnt.booking.msg.export.loading' }));
    await exportBooking(record, formRef, advanceSearch).then((rsp) => {
      hide();
      message.success(intl.formatMessage({ id: 'mnt.booking.msg.export.success' }));

      var fileName = `${intl.formatMessage({ id: 'mnt.booking.title.list' })}-${dateTimestamp()}.xlsx`;
  ...
  const columns: ProColumns<API.RuleListItem>[] = [
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.id" />),
      dataIndex: 'id',
      sorter: true,
    ...
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.userId" />),
      dataIndex: 'userId',
      sorter: true,
      render: (dom, entity, index, action) => {
        var html = <>
          <p>{entity.userId}</p>
          <p>{entity.userName}</p>
          <p>{entity.userPhone}</p>
        </>;
        return html;
      },
    },
    {
      title: <FormattedMessage id="mnt.booking.fields.name.categoryId" />,
    ...
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.appointDate" />),
      dataIndex: 'appointDate',
      sorter: true,
      renderText: (val: string) => `${val}`,
    },
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.appointTime" />),
      dataIndex: 'appointTime',
      sorter: true,
      renderText: (val: string) => `${val}`,
    },
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.phone" />),
      dataIndex: 'phone',
      sorter: true,
      renderText: (val: string) => `${val}`,
    },
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.name" />),
      dataIndex: 'name',
      sorter: true,
      renderText: (val: string) => `${val}`,
    },
    {
      title: (<FormattedMessage id="mnt.booking.fields.name.remark" />),
      dataIndex: 'remark',
      sorter: true,
      width:'18%',
      renderText: (val: string) => `${val}`,
    ...
    // {
    //   title: <FormattedMessage id="mnt.booking.fields.name.dataStatus" />,
    ...
    <PageContainer>
      ...
      <Row>
        <Col span={18} >
          <Space style={{ marginBottom: 10 }}  >
            ...
            <Button onClick={() => { actionRef?.current?.reloadAndRest?.(); }}>{intl.formatMessage({ id: 'mnt.booking.btn.search' })}</Button>
            <a type="primary" onClick={() => { setSearchModalOpen(true); }}  >
              {intl.formatMessage({ id: 'mnt.booking.btn.advSearch' })}
            </a>
            ...
            >
              {intl.formatMessage({ id: 'mnt.booking.btn.restReload' })}
            </a>
          </Space>
        </Col>
        <Col span={6} style={{ textAlign: 'right' }}>
          {/* <Space >
            <Button onClick={() => { setCreateModalOpen(true); }} type='primary'>{intl.formatMessage({ id: 'mnt.booking.btn.create' })}</Button>
      ...
      <ProTable<API.RuleListItem, API.PageParams>
        headerTitle={
          wrapSSR(<>
            <label className={`${className}-table-title ${hashId}`.trim()}>
              {intl.formatMessage({ id: 'mnt.booking.title.list' })}
            </label>
          </>)
        }
        style={{}}
        ...
        request={(params, sort, filter) => {
          return Promise.resolve(listBooking({
            ...params,
