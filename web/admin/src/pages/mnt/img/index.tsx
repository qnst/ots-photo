import { listImg, addImg, deleteImg, updateImg, exportImg } from './service';
...
const Index: React.FC = () => {
  ...
  const Add = async (fields: any) => {
    ...
    return await addImg({ ...fields }).then(rsp => {
      hide();
      messageApi.open({
  ...
  const Update = async (fields: any) => {
    ...
    return await updateImg({ ...fields }).then(rsp => {
      hide();
      messageApi.open({
        type: rsp.success ? 'success' : 'error', content: rsp.msg, duration: 3, className: '-', style: { marginTop: '6vh' },
      });
  ...
  const Delete = async (selectedRows: any) => {
    ...
    return await deleteImg({ ids: selectedRows.map((row: any) => { return { id: row.id } }) }).then(rsp => {
      hide();
      messageApi.open({
        type: rsp.success ? 'success' : 'error', content: rsp.msg, duration: 3, className: '-', style: { marginTop: '6vh' },
      });
  ...
  const Export = async (record: any, formRef: any, advanceSearch: any) => {
    ...
    await exportImg(record, formRef, advanceSearch).then((rsp) => {
      hide();
    ...
    <PageContainer>
      ...
      <ProTable<API.RuleListItem, API.PageParams>
        ...
        request={(params, sort, filter) => {
          return Promise.resolve(listImg({
            ...params,
